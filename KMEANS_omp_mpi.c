...
if (argc != 8)
{
    ...
    fprintf(stderr, "./KMEANS [Input Filename] [Number of clusters] [Number of iterations] [Number of changes] [Threshold] [Output data file] [Number of Threads]\n");
    fflush(stderr);
    MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
}


// Initialize MPI with OpenMP
int provided;
...
//**************************************************
    ...
    if (rank == 0)
    {
        ...
        printf("Number of openMP Threads for each MPI Process: %d\n", threads);
        printf("\n\tData file: %s \n\tPoints: %d\n\tDimensions: %d\n", argv[1], N, D);
    ...
    do
    {
        ...
        *	Assign each point to the nearest centroid.
        ...
        double start_lemmasee  = MPI_Wtime();

        #pragma omp parallel for shared(local_points, local_classMap, centroids, N, D, K) reduction(+: local_changes) 
        for (int i = 0; i < local_n; i++)
        {
        // if (it==1) printf("|%d POINT %d|; ",omp_get_thread_num(),i);
            int class = 1;
            ...
            for (int j = 0; j < K; j++)
            {
                ...
                dist = euclideanDistance(&local_points[i * D], &centroids[j * D], D);
        ...
        double lemmaemmasee = MPI_Wtime();
    tot_lemmaasee += lemmaemmasee -  start_lemmasee;;
        ...
        *    STEP 2: Recalculate centroids (cluster means)
        ...
        int class;
        #pragma omp parallel for shared(local_points, local_classMap, D) private(class) reduction(+: auxCentroids[:K*D], pointsPerClass[:K])
        for (int i = 0; i < local_n; i++)
        {
        ...
        *  Compute the maximum distance between old and new centroids
        ...
        #pragma omp parallel for shared(pointsPerClass, local_centroids, centroid_displs, auxCentroids, K, D)  reduction(max: local_maxDist)
