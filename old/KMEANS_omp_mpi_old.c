 ...
 {  
	...
	if (provided < MPI_THREAD_FUNNELED)
	{
     ...
     do
     {
         ...
         #pragma omp parallel for reduction(+:local_changes) schedule(static,16)
         ...
         {  
             ...
             for (int j = 0; j < K; j++)
             {
                ...
                for (int d = 0; d < D; d++)
                {  
                    diff = (local_points[ppoint + d] - centroids[cpoint + d]);
                    dist += diff*diff;
                }
         ...
         // MPI_Allreduce initiates a non-blocking reduction operation where all processes contribute
