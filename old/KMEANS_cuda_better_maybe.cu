...
* 		- `NULL`
*/
__global__ void step_1_kernel(float* global_data, float* global_centroids, int* global_points_per_class, float* global_aux_centroids, int* global_class_map, int* changes_return_ptr) {
    ...
    if (thread_index < gpu_n) {
        ...
        int class_int = global_class_map[thread_index];
        ...
        if (global_class_map[thread_index] != class_int) {
            atomicAdd(changes_return_ptr, 1);
        }
        
        // Map the value to the class map
        global_class_map[thread_index] = class_int;

        int class_assignment = global_class_map[thread_index];
        atomicAdd(&(global_points_per_class[class_assignment - 1]), 1);

        //if (thread_index == 0)
        //	printf("  PPC: %d\n", points_per_class[thread_index]);

        for (int dim = 0; dim < gpu_d; dim++) {
            int index = (class_assignment - 1) * gpu_d + dim;
            atomicAdd(&global_aux_centroids[index], global_data[data_index + dim]);
        }
    }
}
...
* 		- `NULL`
*/
__global__ void step_2_kernel(float* global_aux_centroids, float* global_centroids, int* global_points_per_class, float* global_distances) {
    // Index of the thread
    int thread_index = (blockIdx.y * gridDim.x * blockDim.x * blockDim.y) + (blockIdx.x * blockDim.x * blockDim.y) +
                            (threadIdx.y * blockDim.x) +
                            threadIdx.x;
    ...
    if (thread_index < gpu_k * gpu_d) {
        ...
        int referring_centroid = (int) thread_index / gpu_d;
        global_aux_centroids[thread_index] /= (float) global_points_per_class[referring_centroid];

        if (thread_index == 0)
            printf("AuxCentr: %f\n", global_aux_centroids[0]);
        
        // Compute the partial distance for a single dimension
        partial_distance = pow((global_centroids[thread_index] - global_aux_centroids[thread_index]), 2);
        atomicAdd(global_distances + referring_centroid, partial_distance);

        global_centroids[thread_index] = global_aux_centroids[thread_index];
        global_aux_centroids[thread_index] = 0.0;
        global_points_per_class[referring_centroid] = 0;
    }
}
...
int main(int argc, char* argv[]) {
    ...
    * */
    ...
    memset(distances, 0.0, k * sizeof(float));
    ...
    CHECK_CUDA_CALL(cudaMalloc((void**) &gpu_distances, k * sizeof(float)));
    CHECK_CUDA_CALL(cudaMemset(gpu_distances, 0.0, k * sizeof(float)));
    ...
    do {
        ...
        step_1_kernel<<<dyn_grid_pts, gen_block, centroids_size>>>(gpu_data, gpu_centroids, gpu_points_per_class, gpu_aux_centroids, gpu_class_map, gpu_changes);
        CHECK_CUDA_LAST();
        ...
        step_2_kernel<<<dyn_grid_cent, gen_block>>>(gpu_aux_centroids, gpu_centroids, gpu_points_per_class, gpu_distances);
        CHECK_CUDA_LAST();
        
        // Copy back distances
        CHECK_CUDA_CALL(cudaDeviceSynchronize());
        CHECK_CUDA_CALL(cudaMemcpy(distances, gpu_distances, k * sizeof(float), cudaMemcpyDeviceToHost));

        //printf("Iteration %d, %d\n", iteration, changes);
        
        for (int i = 0; i < k; i++) {
            printf("Distance[%d]: %f\n", i, distances[i]);
            max_dist = MAX(distances[i], max_dist);
            distances[i] = 0.0;
        }

        sprintf(line,"\n[%d] Cluster changes: %d\tMax. centroid distance: %f", iteration, changes, max_dist);
        output_msg = strcat(output_msg, line);
    } while((changes > min_changes) && (iteration < max_iterations) && (max_dist > pow(max_threshold, 2)));
...
*/
    ...
    if (changes <= min_changes) {
        ...
        printf("\n\nTermination condition:\nMaximum number of iterations reached: %d [%d]", iteration, max_iterations);
    }
    else {
